...
class Database {
  ...
  private initializePool() {
    try {
      ...
      this._connectionError = error instanceof Error ? error.message : "Error desconocido"
    }
  }
  ...
  public async testConnection() {
    if (!this._pool) {
      return {
        connected: false,
        message: this._connectionError || "No se ha inicializado el pool de conexiones",
  ...
  public async agregarPaciente(paciente: any) {
    if (this._useMemory) {
      const newId = this._memoryPacientes.length > 0 ? Math.max(...this._memoryPacientes.map((p) => p.id)) + 1 : 1
  ...
  public async agregarOdontologo(odontologo: any) {
    if (this._useMemory) {
      const newId = this._memoryOdontologos.length > 0 ? Math.max(...this._memoryOdontologos.map((o) => o.id)) + 1 : 1
  ...
  public async getConsultas() {
    if (this._useMemory) {
      ...
      return this._memoryConsultas.map((consulta) => {
        const paciente = this._memoryPacientes.find((p) => p.id === consulta.paciente_id)
        const odontologo = this._memoryOdontologos.find((o) => o.id === consulta.odontologo_id)
        ...
        return {
          ...
          paciente_cedula: paciente ? `${paciente.tipo_cedula}-${paciente.cedula}` : "",
          odontologo: odontologo ? `${odontologo.nombre} ${odontologo.apellido}` : "No asignado",
          ...
          for (const sector of sectoresRows) {
            ...
            for (const diente of dientesRows) {
              ...
              for (const segmento of segmentosRows) {
                odontodiagrama[sectorKey].dientes[dienteKey].segmentos[segmento.numero_segmento] =
  ...
  public async agregarConsulta(consulta: any) {
    if (this._useMemory) {
      const newId = this._memoryConsultas.length > 0 ? Math.max(...this._memoryConsultas.map((c) => c.id)) + 1 : 1
    ...
    try {
      ...
      `,
        [consulta.paciente_id, consulta.odontologo_id, historialId, consulta.motivo],
      )
  ...
  public async actualizarConsulta(id: number, consulta: any) {
    if (this._useMemory) {
      ...
      // Obtener datos del paciente y odontólogo para el retorno
      const paciente = this._memoryPacientes.find((p) => p.id === this._memoryConsultas[index].paciente_id)
      const odontologo = this._memoryOdontologos.find((o) => o.id === this._memoryConsultas[index].odontologo_id)
  ...
  //     return {
  //       connected: false,
  //       message: "Usando datos en memoria. No hay conexión a la base de datos.",
  //     }
  //   }
